...
function updatePaths(trigger) {
    ...
    let t = graphLines.attr("stroke", getColor_AC).call(redrawLine)
        .filter(c=>c.p.isTarget)
        .attr("class", "target");
    let pfb = graphLines.filter(c=>c.p.isPrefBounds);
    pfb.style("stroke-dasharray", "2, 9");

    if (targetDashed) t.style("stroke-dasharray", "8, 6");
...
function updatePhoneTable() {
    ...
    if (allowSquigDownload) {
        function saveSquiggle(p) {
            ...
            // this code is if you want to download all channels visible. works best if you have "choose download location" disabled in your browser, else you need fast hands
            // let squigUrls = [];
            // channels.forEach((c,i) => {
            //     let squigName = p.fileName + " " + LR[i];
            //     p.comp ? squigName += " - " + p.comp : null;
            //     let filename = squigName + ".txt";
            //     let data = c.map(d => d.join("\t")).join("\n");
            //     let blob = new Blob([data], {type: "text/plain; charset=utf-8"});
            //     let url = URL.createObjectURL(blob);
            // 
            //     squigUrls.push({url:url, filename:filename});
            // });
            // 
            // let interval = setInterval(() => {
            //     let squigUrl = squigUrls.shift();
            //     if (squigUrl) {
            //         squig.href = squigUrl.url;
            //         squig.download = squigUrl.filename;
            //         squig.click();
            //         URL.revokeObjectURL(squigUrl.url);
            //     } else {
...
d3.json(typeof PHONE_BOOK !== "undefined" ? PHONE_BOOK
    ...
    updateDF = (boost, tilt, ear, treble, change) => {
        ...
        // Bass Shelf
        let filters = [
            {disabled: false, type:"LSQ", freq:105, q:0.71, gain:boost},
