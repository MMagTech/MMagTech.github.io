...
const init_phones = ["MMagTech Target"],                // Optional. Which graphs to display on initial load. Note: Share URLs will override this set
      ...
      watermark_text = "mmagtech.sguig.link",                          // Optional. Watermark appears behind graphs
      watermark_image_url = "img/mmagtech.png",                     // Optional. If image file is in same directory as config, can be just the filename
      page_title = "Frequency Responses - Squiglink by MMagTech",
      ...
      targetColorCustom = "green",                    // If false, targets appear as a random gray value. Can replace with a fixed color value to make all targets the specified color, e.g. "black"
...
const targets = [
    { type:"Preference",    files:["Bad Guy","MMagTech","nymz","PaulWasabi","Precogvision","Rtings","Sonarworks","Super Review 21","Super Review 22"] },
    { type:"Reference",   files:["Diffuse Field","Etymotic","IEF Neutral","Harman 2019v2"] }
];
...
function watermark(svg) {
    ...
    if ( watermark_text ) {
        wm.append("text")
            .attrs({x:229, y:161, "font-size":16, "text-anchor":"middle", "class":"graph-name"})
            .text(watermark_text);
    } 
}
